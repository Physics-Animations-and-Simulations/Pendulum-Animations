...
Date: 15/03/2025
Description: This script creates a simple simulation of the motion of the Dumped Pendulum.
...
l = 5       # length of rod (m)
m = 5       # mass (kg)
b = 0.5     # dumping coefficient (kg/s)

def DumpedPemdulum(t, y):
    theta, z = y
    dtheta_dt = z
    dz_dt =  -(b/(m * np.sqrt(l)))*z - (g/l)*np.sin(theta)
...
sol = solve_ivp(DumpedPemdulum, t_span, y0, t_eval=t_eval)
theta = sol.y[0]
time = sol.t
